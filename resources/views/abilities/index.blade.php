...
<html lang="{{ app()->getLocale() }}">
	...
	<body>
		...
		<div><br><br><br></div>
		<div class="col-md-12">
			<h1 class="text-center" style="font-size: 30px"><i class="fas fa-puzzle-piece"></i> Lista de Competencias</h1>
			<hr>
			<ol class="breadcrumb">
	            <li class="breadcrumb-item"><a href="{{url('home')}}">Inicio</a></li>
	            <li class="breadcrumb-item active" aria-current="page">Lista Competencias</li>
	        </ol>
			<a href="{{ url('abilities/create')}}" class="btn btn-outline-info">
				<i class="fa fa-plus"></i> Agregar Competencia
			</a>
			<a href="{{url('abilities/pdf')}}" class="btn btn-outline-primary">
				<i class="fa fa-file-pdf">  Exportar</i>
			</a>
			<a href="{{url('abilities/excel')}}" class="btn btn-outline-success">
				<i class="fa fa-file-excel">  Exportar</i>
			</a>
			<hr>
			<form class="form-inline" action="{{ url('abilities/search') }}">
			...
			<table class="table table-striped table-hover text-center" style="font-size: 12px;">
				<thead class="thead-dark" style="font-size: 12px;">
					<tr>
						<th>Competencia</th>
						<th>Programa de<br>Formación</th>
						<th>Cantidad<br>Horas</th>
						<th>Acciones</th>
					</tr>
				</thead>
				<tbody class="results">
    ...
    <script>
    	$(document).ready(function(){
            ...
            $('form').on('keyup','#csearch', function(event) {
                event.preventDefault();
                $name=$(this).val();
                $.get('abilities/ajaxsearch',{name:$name}, function(data){
                    $('.results').html(data);
                });
            });
